...
class AnswerSearcher:
    ...
    def answer_prettify(self, question_type, answers):
        ...
        if question_type == 'figure_age':
            ...
            subject = answers[0]['m.name']
            final_answer = '{0}——作品为：{1}'.format(subject, '；'.join(list(set(desc))[:self.num_limit]))
            ...
            desc = [i['m.pre_emperor'] for i in answers if i['m.next_emperor'] is not None]
            subject = answers[0]['m.name']
            final_answer = '{0}的前任皇帝是：{1}'.format(subject, '；'.join(list(set(desc))[:self.num_limit]))

        elif question_type == 'figure_next_emperor':
            desc = [i['m.next_emperor'] for i in answers if i['m.next_emperor'] is not None]
            subject = answers[0]['m.name']
        ...
        elif question_type == 'figure_by_age':
        ...
        elif question_type == 'figure_pre_next_emperor':
            desc = [i['m.name'] for i in answers]
